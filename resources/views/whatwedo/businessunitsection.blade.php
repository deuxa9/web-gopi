<div class="container text-start desktop-show">
    <div class="mb-3">
        <h1 class="fw-bold fs-1 text-center text-dark">Business Unit</h1>
    </div>

    <div class="card-group border-0 rounded-0">
      <div class="card mr-3 my-2">
        <img src="assets/img/sector1.png" class="card-img-top" alt="...">
        <h3 class="position-absolute bottom-0 start-50 translate-middle-x mb-5 fw-bold text-white">
          Property and Construction
        </h3>
      </div>
      <div class="card mr-3 my-2">
        <img src="assets/img/sector2.png" class="card-img-top" alt="...">
        <h3 class="position-absolute bottom-0 start-50 translate-middle-x mb-5 fw-bold text-white">
          Technology and Innovation
        </h3>
      </div>
      <div class="card mr-3 my-2">
        <img src="assets/img/sector3.png" class="card-img-top" alt="...">
        <h3 class="position-absolute bottom-0 start-50 translate-middle-x mb-5 fw-bold text-white">
          Digital Marketing and Branding
        </h3>
      </div>
    </div>

    <div class="card-group border-0 rounded-0">
      <div class="card mr-3 my-2">
        <img src="assets/img/sector4.png" class="card-img-top" alt="...">
        <h3 class="position-absolute bottom-0 start-50 translate-middle-x mb-5 fw-bold text-white">
          Food and Beverage
        </h3>
      </div>
      <div class="card mr-3 my-2">
        <img src="assets/img/sector5.png" class="card-img-top" alt="...">
        <h3 class="position-absolute bottom-0 start-50 translate-middle-x mb-5 fw-bold text-white">
          Gadget and Wearables
        </h3>
      </div>
      <div class="card mr-3 my-2">
        <img src="assets/img/sector6.png" class="card-img-top" alt="...">
        <h3 class="position-absolute bottom-0 start-50 translate-middle-x mb-5 fw-bold text-white">
          Pre-seed <br> Venture
        </h3>
      </div>
    </div>
</div>
<div class="text-start mobile-show">
    <div class="mb-3">
        <h1 class="fw-bold fs-1 text-center text-dark">Business Unit</h1>
    </div>

    <div class="card-group border-0 rounded-0">
      <div class="card border-0 mb-0">
        <img src="assets/img/sector1.png" class="card-img-top" alt="...">
        <h3 class="position-absolute bottom-0 start-50 translate-middle-x mb-5 fw-bold text-white">
          Property and Construction
        </h3>
      </div>
      <div class="card border-0 mb-0">
        <img src="assets/img/sector2.png" class="card-img-top" alt="...">
        <h3 class="position-absolute bottom-0 start-50 translate-middle-x mb-5 fw-bold text-white">
          Technology and Innovation
        </h3>
      </div>
      <div class="card border-0 mb-0">
        <img src="assets/img/sector3.png" class="card-img-top" alt="...">
        <h3 class="position-absolute bottom-0 start-50 translate-middle-x mb-5 fw-bold text-white">
          Digital Marketing and Branding
        </h3>
      </div>
    </div>

    <div class="card-group border-0 rounded-0">
      <div class="card border-0 mb-0">
        <img src="assets/img/sector4.png" class="card-img-top" alt="...">
        <h3 class="position-absolute bottom-0 start-50 translate-middle-x mb-5 fw-bold text-white">
          Food and Beverage
        </h3>
      </div>
      <div class="card border-0 mb-0">
        <img src="assets/img/sector5.png" class="card-img-top" alt="...">
        <h3 class="position-absolute bottom-0 start-50 translate-middle-x mb-5 fw-bold text-white">
          Gadget and Wearables
        </h3>
      </div>
      <div class="card border-0 mb-0">
        <img src="assets/img/sector6.png" class="card-img-top" alt="...">
        <h3 class="position-absolute bottom-0 start-50 translate-middle-x mb-5 fw-bold text-white">
          Pre-seed <br> Venture
        </h3>
      </div>
    </div>
</div>